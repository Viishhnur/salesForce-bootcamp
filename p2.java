...
class p2{
    ...
    public static int islandPerimeter(int[][] grid) {
        ...
        for(int i=0;i<m;i++){
            for(int j=0;j<n;j++){
                if(grid[i][j] == 1){
                    // Start checking nbr cells
                    

                    if(i-1 < 0 || (i-1>=0 && grid[i-1][j] == 0)) perimeter++; // up
                    if(i+1 >= m || (i+1<m && grid[i+1][j] == 0)) perimeter++; // down
                    if(j-1 < 0 || (j-1>=0 && grid[i][j-1] == 0)) perimeter++; // left
                    if(j+1 >= n || (j+1<n && grid[i][j+1] == 0)) perimeter++; // right
